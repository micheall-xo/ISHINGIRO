...
const HARD_CODED_QUICK_ACCESS = [
	{ id: 'homework', title: 'Homework', subtitle: 'View assignments', icon: '📚', color: '#3b82f6', route: '/student/homework' },
	{ id: 'results', title: 'Results', subtitle: 'Check grades', icon: '📈', color: '#10b981', route: '/student/results-list' },
	{ id: 'messages', title: 'Messages', subtitle: 'Chat securely', icon: '💬', color: '#2563eb', route: '/student/messages' },
	{ id: 'notices', title: 'Notices', subtitle: 'School updates', icon: '📢', color: '#f59e0b', route: '/student/notices' },
	{ id: 'exam-routine', title: 'Exam Routine', subtitle: 'Exam schedule', icon: '📅', color: '#8b5cf6', route: '/student/exam-routine' },
	{ id: 'ask', title: 'Ask Teacher', subtitle: 'Get help', icon: '❓', color: '#ef4444', route: '/student/ask' },
	{ id: 'profile', title: 'Profile', subtitle: 'Student info', icon: '👤', color: '#06b6d4', route: '/student/profile' }
];


	export default function StudentHome() {
	usePageData('student-index');
	...
	const user = {
		...
		grade: profile?.studentInfo?.grade
			? `${profile.studentInfo.grade}${profile.studentInfo.section ? ` ${profile.studentInfo.section}` : ''}`
			: 'Grade - N/A',
		avatar: ((profile?.fullName || 'Student').match(/\b\w/g) || []).slice(0, 2).join('').toUpperCase() || 'ST',
	...
	const attendance = profile?.studentInfo?.attendance;
	const totalDays = attendance?.totalDays || 0;
	const presentDays = attendance?.presentDays || 0;
	const attendancePct = totalDays > 0 ? Math.round((presentDays / totalDays) * 100) : 0;
	const gpa = profile?.studentInfo?.performance?.gpa ?? 0;
	const balance = profile?.studentInfo?.pocketMoney?.balance ?? 0;

	const quickStats = [
		{ label: 'GPA', value: String(gpa), color: '#3b82f6' },
		{ label: 'Balance', value: `$${Number(balance).toFixed(2)}`, color: '#ef4444' },
