...
export default function ResultSecond() {
  ...
  useEffect(() => {
    (async () => {
      try {
        ...
        const second = terms.find((t) => String(t.term || '').toLowerCase().includes('second')) || terms[1] || null;
        setTerm(second);
        setYearly(response?.yearly || {});
        setStudent(response?.student || {});
      } catch (error) {
        Alert.alert('Second Term', error.message || 'Failed to load second term result');
      } finally {
        setLoading(false);
      }
    })();
  ...
  const description = useMemo(() => {
    if (!term) return 'No second-term report found in the database.';
    return `Overall ${term.overallGrade || 'N/A'} (${Number(term.overallScore || 0).toFixed(2)}%). Yearly status: ${
      yearly.status || 'N/A'
    }, GPA: ${Number(yearly.gpa || 0).toFixed(2)}.`;
  }, [term, yearly]);
    ...
    <View style={styles.container}>
      <View style={styles.headerCard}>
        ...
        <View style={styles.metaRow}>
          <Text style={styles.metaText}>Term : Second</Text>
        ...
        {loading ? (
          ...
          (term?.subjects || []).slice(0, 5).map((s) => (
            <Text key={s.name} style={styles.subjectLine}>
              {s.name}: Formative {s.formativeScore ?? '-'} | Exam {s.examScore ?? '-'} | Final{' '}
