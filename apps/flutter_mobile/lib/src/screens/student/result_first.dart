...
class StudentResultFirstScreen extends StatefulWidget {
  const StudentResultFirstScreen({super.key, required this.auth});

  final AuthController auth;

  @override
  State<StudentResultFirstScreen> createState() => _StudentResultFirstScreenState();
}

class _StudentResultFirstScreenState extends State<StudentResultFirstScreen> {
  bool _loading = true;
  String _error = '';
  ...
  Future<void> _load() async {
    ...
    try {
      ...
      final first = terms
          ...
          .firstWhere(
            (t) => '${t['term']}'.toLowerCase().contains('first'),
            orElse: () => terms.isNotEmpty ? (terms.first as Map).cast<String, dynamic>() : <String, dynamic>{},
          );
      _term = first.isEmpty ? null : first;
  ...
  Widget build(BuildContext context) {
    ...
    final desc = term == null
        ? 'No first-term report found in the database.'
    ...
    return Scaffold(
      appBar: AppBar(title: const Text('First Term Result')),
              ...
              child: Padding(
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text('Term: First', style: const TextStyle(color: Color(0xFFEFF6FF))),
